...
public class GUI implements ActionListener
{
    ...
    public void actionPerformed(ActionEvent e)
    {
        ...
        if(e.getActionCommand().equals("TRAIN")){
            if (agent == null) {
                ...
                for (int i = 0; i < QLearningAgent.ITERATIONS; i++) {
                    agent.train();
                    updateGame();
                    
                    // Restart the map for further learning.
                    if (w.gameOver()) {
                        startMap(seed);
                    }
                }
    ...
    private void startMap(int seed) {
        ...
        if (s.equalsIgnoreCase("Random")) {
            w = MapGenerator.getRandomMap(seed).generateWorld();
        } else {
            int i = Integer.parseInt(s);
            w = maps.get(i-1).generateWorld();
        }
